 ...
 * @date 2016-09-21
 */
public class ASLoadingView extends View {
    ...
    public ASLoadingView(Context context) {
        this(context, null);
    }

    public ASLoadingView(Context context, AttributeSet attrs) {
        this(context, attrs, R.attr.ASLoadingStyle);
    }

    public ASLoadingView(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        TypedArray array = getContext().obtainStyledAttributes(attrs, R.styleable.ASLoadingView, defStyleAttr, 0);
        mSize = array.getDimensionPixelSize(R.styleable.ASLoadingView_loading_view_size, SizeHelper.dp2px(context, 32));
        mPaintColor = array.getInt(R.styleable.ASLoadingView_android_color, Color.WHITE);
        array.recycle();
        initPaint();
    }

    public ASLoadingView(Context context, int size, int color) {
        super(context);
        mSize = size;
        mPaintColor = color;
        initPaint();
